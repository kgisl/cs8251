...
enum days0 {SUN, MON, TUE, WED, THU, FRI, SAT};
// SUN gets 0, MON gets 1, TUE gets 2, etc.

// Enumeration values can also be specified
enum days {SUNDAY = 1, MONDAT, TUESDAY, WEDNESDAY = 99, THURSDAY, FRIDAY, SATURDAY};
...
int main (int argc, char** argv)
{
  ...
  int jdx = 0;
  int s = jdx++; // Return j THEN increase j. (s = 0, j = 1)
  s = ++jdx; // Increase j THEN return j. (s = 2, j = 2)
  // same with j-- and --j
  ...
  */
  ...
  size_t size_a = 10;
  int *my_arr = calloc(size_a, sizeof(int));
  // Add an element to the array
  size_a++;
  my_arr = realloc(my_arr, sizeof(int) * size_a);
  if (my_arr == NULL) {
